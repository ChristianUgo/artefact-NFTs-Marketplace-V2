...
export default function Home() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setNotice({ type: "success", text: "NFT minted and listed on the local chain." }); setView("market"); await load();
  ...
  async function buy(item) {
    if (!account) return wallet();
    if (item.seller.toLowerCase() === account.toLowerCase()) return setNotice({ type: "error", text: "Switch to Hardhat Account #1 to buy your own listing." });
  ...
  return <main>
    ...
    <div className="network"><i /> Hardhat local network <span>Chain 31337</span><b>Contract 0x5FbD...0aa3</b></div>
    {notice && <div className={`notice ${notice.type}`}><span>{notice.text}</span><button onClick={() => setNotice(null)}>Close</button></div>}

    {view === "market" && <>
      <section className="hero">
        <div><p className="eyebrow">A LOCAL NFT STUDIO</p><h1>Make the work. <em>Keep the signal.</em></h1><p className="lede">An original marketplace interface for testing digital collections on your local Ethereum network.</p><div className="actions"><button className="primary" onClick={() => setView("create")}>Create an NFT</button><button className="link" onClick={load}>Refresh marketplace</button></div></div>
      ...
      <section className="market-section"><div className="section-title"><div><p className="eyebrow">MARKETPLACE</p><h2>Collectible listings</h2></div><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search your local NFTs" /></div>
      {loading ? <Empty text="Loading local contract listings..." /> : results.length ? <div className="grid">{results.map((item) => <Card key={item.tokenId} item={item} choose={setSelected} />)}</div> : <Empty text="No local NFTs are listed yet." button={() => setView("create")} />}</section>
    </>}
...
function Dialog({ item, account, close, buy, busy }) { const ownListing = !item.sold && account && item.seller.toLowerCase() === account.toLowerCase(); return <div className="backdrop" onMouseDown={close}><article className="dialog" onMouseDown={(event) => event.stopPropagation()}><button className="close" onClick={close}>Close</button><div className="dialog-art">{item.image ? <img src={item.image} alt={item.name} /> : <span>#{item.tokenId}</span>}</div><div className="details"><p className="eyebrow">LOCAL EDITION #{item.tokenId}</p><h2>{item.name}</h2><p>{item.description}</p><div className="facts"><span>{item.sold ? "Owned by" : "Listed by"} <b>{shortAddress(item.sold ? item.owner : item.seller)}</b></span><strong>{item.price} ETH</strong></div>{item.sold ? <p className="owner">Owned by the connected wallet. Resale is not enabled in this interface yet.</p> : ownListing ? <p className="owner">This is your listing. Switch to another Hardhat account to test a purchase.</p> : <button className="primary full" disabled={busy} onClick={() => buy(item)}>{busy ? "Waiting..." : `Buy for ${item.price} ETH`}</button>}</div></article></div>; }
